...
# fit generalized linear mixed effects model (GLMM)
glmm_incomp <- glmer(chosen_voicing ~ correct_voicing +
...
# fit null model
glmm_incomp_null <- glmer(chosen_voicing ~ 1 +
                            (1 + correct_voicing | voice) +
                            (1 + correct_voicing | listener) +
                            (1 + correct_voicing | item_pair),
                          data = incomplete,
...
# run anova analysis
anova(glmm_incomp_null, glmm_incomp)

# When correct_voicing (intercept) is 0 (0 = voiced), the log odds of choosing the intended voicing category (thus chosen_voicing = voiced) are -0.02. 
# For each unit of correct_voicing, the log odds of choosing the intended voicing category increase by 0.52 (SE = 0.17). 
# (Though the correct_voicing can really only increase by one unit, as it can either be 0 = voiced or 1 = voiceless.) 
#E.g., if correct_voicing is at 1 (1 = voiceless), the log odds of choosing the intended voicing category (thus chosen_voicing = voiceless) are 0.50.
# (So in plain English, this means that there's a higher probability of choosing the intended voicing category if the underlying form is voiceless.)
# At an alpha level of 0.05 (⍺ <= 0.05), this effect was significant. The null hypothesis can be rejected as the test statistics or more extreme test statistics are sufficiently improbable (χ2(1) = 8.18, p = 0.004).
...
preds_incomp <- predict_response(glmm_incomp, 
                                 terms = "correct_voicing") |>
  rename("correct_voicing" = x)

# plot
plot_pred_incomp <-
  ggplot(data = preds_incomp,
  ...
  # add titles
  labs(title = "Model estimates for chosen voicing as a function of correct (intended) voicing",
