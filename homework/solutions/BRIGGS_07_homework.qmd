...
pacman::p_load(rstudioapi, tidyverse, lme4, brms, bayestestR, bayesplot, tidybayes, modelr)
...
# get priors (for model set in code chunck above)
get_prior(quant_lmer, quantese)
...
mcmc_plot(xmdl_quant, type = "trace")
  # I unfortunately couldn't make it to the lecture last week but the plots look similar to the   examples in slides_07, so I assume that they're good
# plot denisty estimates
mcmc_dens_overlay(xmdl_quant, pars = "b_log_frequency")
  # the density estimate curves for all chains appear to have approximately the same height and   width (like the example in the slides_07), so I assume that they're good
# check model output
xmdl_quant
  # Rhat = 1.01 is acceptable in terms of chain convergence(?). Tail_ESS (638 > 100) looks "safe", but Bulk_ESS (296 < 400) is a bit low.

# increase warmup period to fix low Bulk_ESS
xmdl_quant <- brm(quant_lmer,
                  family = gaussian(),
                  prior = my_priors,
                  data = quantese,
...
# re-run convergence diagnostics
# re-plot Markov chain Monte Carlo (MCMC) simulations
mcmc_plot(xmdl_quant, type = "trace")
  # trace plots still look good
# re-plot denisty estimates
mcmc_dens_overlay(xmdl_quant, pars = "b_log_frequency")
  # density estimate curves still look good
# check new model output
xmdl_quant
  # Rhat = 1.00 suggests that the chains have converged (i.e., proper convergence of model).      Tail_ESS has increased from 638 to 1366, and Bulk_ESS (613 > 400) is no longer too low.
```
...
# create data grid with 100 evenly spaced values of continuous predictor
grid_quant <- quantese |> 
  data_grid(log_frequency = seq_range(log_frequency, 100))

# extract posterior draws from model
draws_quant <- xmdl_quant |>
  spread_draws(b_Intercept, b_log_frequency) |>
  # evaluate linear predictor at each value of log_frequency
  crossing(grid_quant) |>
  group_by(log_frequency) |> 
  # calculate mean and 95% CrI (seperated into 50%, 80% and 95% intervals)
  mean_qi(mean = b_Intercept + b_log_frequency * log_frequency,
          .width = c(.5, .8, .95))
  
# plot posterior mean and 95% CrI
draws_quant |> 
  ggplot(aes(x = log_frequency, 
             y = mean, 
             ymin = .lower, 
             ymax = .upper)) +
  geom_lineribbon(color = "white") +
  scale_fill_brewer(palette = "Purples") +
  guides(fill = "none") +
  labs(y = "Predicted log(RT)",
       x = "log(frequency)",
