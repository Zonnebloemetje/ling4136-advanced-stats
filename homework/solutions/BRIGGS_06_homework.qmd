...
sim |>
  ggplot(aes(x = Distance,
             fill = Sem)) +
  # select plot type, set transparency and remove outline
  geom_density(alpha = 0.5,
               na.rm = TRUE,
               color = NA) +
   # set x-axis titles
 xlab("Raw distance (mm)") +
  # set theme
  theme_minimal() +
  # select colors manually
  scale_fill_manual(values = c("maroon1", 
                               "deepskyblue3"))

# SemDifferent: Normal distribution (with one major and two minor modes). Most values for the distance between semantically different city names appear to fall roughly between 60 and 90 mm.

# SemSimilar = Positive skew (with one major and one minor mode). Most values for the distance between semantically similar city names appear to fall roughly between 40 and 90 mm.
```
...
my_priors <- c(prior(normal(107, 35), class = 'Intercept'),
...
# The model assumes a more normal distribution than what's actually represented in the data. It also overestimates the probability of values around 110 mm and underestimates the probability of values around the two lower modes.
